...
public class BoardController {
    ...
    public String view_main_board(
            Model model,
            @SessionAttribute(value = "loginedUser", required = false) UserDTO userDTO
    ){
        log.info("/board/main - GET");
        // Session에 유저가 없다는 것은 로그인 하지 않고 왔다는 의미
        if(!login_check(userDTO)) return "redirect:/main";
        // 로그인 되어있다면
...
//    *************************************************************
    private boolean login_check(UserDTO userDTO){
        if(userDTO == null){
